...
float Vector3::angleR(Vector3& v2) {
	//returns angle between 2 vectors in radian
	return (acosf((this->dot(v2)) / (this->length() * v2.length()))); 
}

float Vector3::angleD(Vector3& v2){
	//returns angle between 2 vectors in degrees
	return (acosf(this->dot(v2) / (this->length() * v2.length()))) * 180 / 3.1415;
}
